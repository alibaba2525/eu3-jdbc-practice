...
public class Pojo_deserialize {
    ...
    public void oneSpartanPojo(){
        ...
        Response response = given().accept(ContentType.JSON)
                .pathParam("id", 15)
                .when().get("http://18.204.13.78:8000/api/spartans/{id}");

        assertEquals(response.statusCode(),200);

        //json to our Spartan class object (json to POJO(custom class) basically)
        Spartan spartan15 = response.body().as(Spartan.class);//setting is done by this as method!
